...
class ActionSubtask(PromptTask):
    ...
    def action_schema(cls, action_types: list) -> Schema:
        return Schema(
            ...
            schema={
                Literal("type", description="Action type"): schema.Or(
                    *action_types
                ),
        ...
        try:
            if self.action_name == "error":
                ...
                if self.action_type == "tool":
                    ...
                    if self._memory:
                        observation = getattr(
                            self._memory, self.action_activity
                        )(self.action_input)
                    else:
    ...
    def __init_from_prompt(self, value: str) -> None:
        ...
        if len(action_matches) > 0:
            try:
                ...
                if self.action_type == "tool":
                    if self.action_name:
                        self._tool = self.origin_task.find_tool(
                            self.action_name
                        )

                    if self._tool:
                        self.__validate_action_input(
                            self.action_input, self._tool
                        )
                elif self.action_type == "memory":
                    if self.action_name:
                        self._memory = self.origin_task.find_memory(
                            self.action_input["values"]["memory_name"]
                        )

                    if self._memory:
                        self.__validate_action_input(
                            self.action_input, self._memory
                        )
